...
public class CompanyPage extends TestBase {
	UIUtil util = new UIUtil(driver, testReport);
	...
	@FindBy(id = "membershipType")
	WebElement membershipLevel;
	
	@FindBy(xpath = "//a[contains(text(),'Generate Membership Invoice')]")
	WebElement generateMembershipInvoice;
	
	@FindBy(id = "companyEditUpdateBtn")
	WebElement companyEditUpdateBtn;
	...
	public void selectMembershipLevel(String membershipType) {
		util.selectByText(membershipLevel, membershipType, "Select Membership Level under the dropdown");
	}
	
	public void clickOnMenerateMembershipInvoiceLink() {
		util.getWaitUtil().waitForElementToBeEnable(generateMembershipInvoice);
		util.click(generateMembershipInvoice, "Click on Generate Membership Invoice link");
	}
	
	public void clickOnUpdateBttnAfterScrollingUp() {
		util.pageScrollToUpInSlowMotion();
		util.click(companyEditUpdateBtn, "Click on Search Users link at breadcrumb");
	}
}
